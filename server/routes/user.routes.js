...
export default function userRoutes(pool) {
  ...
  userRouter.get(
    ...
    async (req, res) => {
      try {
        const { id } = req.params;
        const user = await localUserModel.getUserById(id);
      ...
      try {
        ...
        if (userId == id) {
          ...
          if (password != undefined) {
            ...
            updatedUser = await localUserModel.updateUserNameAndPassword(
              firstname,
              lastname,
              hashedPassword,
              userId
            ...
            updatedUser = await localUserModel.updateUserName(
              firstname,
              lastname,
              userId
            );
